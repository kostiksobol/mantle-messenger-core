import { arrayBufferToBase64, base64ToArrayBuffer, encoder, randomBytes, } from "./encoding";
...
async function importHmacKey(chatKeyBase64) {
...
async function hmacSha256Base64(chatKeyBase64, message) {
...
export async function deriveChatId(chatKeyBase64) {
    return hmacSha256Base64(chatKeyBase64, CHAT_ID_DOMAIN);
}
export async function assertChatId(chatKeyBase64, chatId) {
...
export function parseMessageTag(tag) {
...
async function signMessageTagNonce(chatKeyBase64, nonceBase64) {
    return hmacSha256Base64(chatKeyBase64, `${MESSAGE_TAG_DOMAIN}:${nonceBase64}`);
}
export async function generateMessageTag(chatKeyBase64) {
    const nonce = arrayBufferToBase64(randomBytes(MESSAGE_TAG_NONCE_BYTES).buffer);
    const mac = await signMessageTagNonce(chatKeyBase64, nonce);
    return `${nonce}.${mac}`;
}
export async function isMessageTagForChat(chatKeyBase64, tag) {
    const parsed = parseMessageTag(tag);
    if (!parsed) {
        return false;
    }
    const expectedMac = await signMessageTagNonce(chatKeyBase64, parsed.nonce);
