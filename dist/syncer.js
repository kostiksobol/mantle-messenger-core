...
import { MAIN_CONNECTOR_ADDRESS, ZERO_ADDRESS, mainConnectorAbi, userContractAbi, } from "./contracts";
import { parseChatEventPayload, parseMainInvitationPayload, } from "./protocol/payloads";
...
export function startBlockchainSyncer(options) {
...
class BlockchainSyncer {
    ...
    stopped = false;
    mainUnwatch;
    userContractsUnwatch;
    watchedUserContractsKey = "";
    running = new Set();
    pending = new Set();
    constructor(options) {
        this.ownerAddress = asAddress(options.ownerAddress);
        this.publicClient = options.publicClient;
        const mainConnectorAddress = options.mainConnectorAddress || MAIN_CONNECTOR_ADDRESS;
    ...
    async refreshUserContractWatcher() {
        const members = await db.chatMembers
            .where("ownerAddress")
            .equals(this.ownerAddress)
            .toArray();
        const addresses = Array.from(new Set(members.map((member) => asAddress(member.userContract)))).sort();
    ...
    async putSelfProfile(chainUser) {
        ...
        try {
            ...
            await db.selfProfiles.put({
                ...profile,
                id: latest?.id,
                mainRecordsCursor: latest?.mainRecordsCursor ?? existing?.mainRecordsCursor ?? 0,
            });
        }
    }
    async syncMainRecords() {
    ...
    async processMainRecord(record) {
        const keys = loadRsaKeyPair(this.ownerAddress);
        if (!keys) {
            return;
        }
    ...
    async syncAllKnownUserContracts() {
        while (!this.stopped) {
            ...
            const members = await db.chatMembers
                ...
                .equals(this.ownerAddress)
                .toArray();
            const userContracts = Array.from(new Set(members.map((member) => asAddress(member.userContract))));
    ...
    async syncUserContract(userContract) {
        const members = await db.chatMembers
            .where("userContract")
    ...
    async processUserContractMessage(input) {
        ...
        const isForChat = await isMessageTagForChat(chat.chatKey, input.message.tag);
        if (!isForChat) {
            await this.setMemberCursor(input.member, input.sourceMessageIndex + 1);
            return;
        }
        let plaintext;
        try {
            plaintext = await aesDecrypt(chat.chatKey, input.message.encryptedContent);
        }
        catch {
            await this.setMemberCursor(input.member, input.sourceMessageIndex + 1);
            return;
        ...
        if (payload.event === "ChatCreation") {
            if (normalizeAddress(input.member.userAddress) ===
    ...
    async addUserToChatByAddress(input) {
    ...
    async setMemberCursor(member, cursor) {
        await db.chatMembers.put({
            ...member,
            cursor,
        });
    }
    async putMessageIfNew(input) {
