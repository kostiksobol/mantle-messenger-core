...
import { deriveChatId, generateChatKey, generateMessageTag, } from "../crypto/hmac";
...
import { MAX_CACHED_ATTACHMENT_SIZE_BYTES, putCachedAttachmentFile, } from "../ipfs/attachmentCache";
import { normalizeAddress } from "../db";
import { ensureRsaKeyPair } from "../localKeys";
import { ZERO_ADDRESS, mainConnectorAbi, userContractAbi, } from "../contracts";
import { createChatCreationPayload, createInvitationPayload, createMainInvitationPayload, createMessagePayload, encodePayload, } from "../protocol/payloads";
function chainUserFrom(value) {
    const item = value;
    return {
        userAddress: (item.userAddress ?? item[0]),
        login: String(item.login ?? item[1] ?? ""),
        name: String(item.name ?? item[2] ?? ""),
        pubkey: String(item.pubkey ?? item[3] ?? ""),
        userContract: (item.userContract ?? item[4]),
...
async function wait(ctx, hash) {
    await ctx.publicClient.waitForTransactionReceipt({ hash });
}
async function readUserByAddress(ctx, userAddress) {
    const user = chainUserFrom(await ctx.publicClient.readContract({
...
async function readUserByLogin(ctx, userLogin) {
    const user = chainUserFrom(await ctx.publicClient.readContract({
        address: ctx.mainConnectorAddress,
        abi: mainConnectorAbi,
...
async function uploadFileAttachment(chatKey, file) {
...
export async function createChat(ctx, input) {
    const keys = await ensureRsaKeyPair(ctx.ownerAddress);
    const chatKey = generateChatKey();
    const chatId = await deriveChatId(chatKey);
    const creationBox = await aesEncrypt(chatKey, encodePayload(createChatCreationPayload({
        name: input.name.trim() || "Unnamed chat",
    })));
    const creationTag = await generateMessageTag(chatKey);
    const mainInvitation = await rsaEncrypt(keys.publicKey, encodePayload(createMainInvitationPayload({
        chatKey,
        creator: ctx.ownerAddress,
    })));
    const batched = (await ctx.transactions.sendCallsBatch?.([
        {
            to: ctx.selfProfile.userContract,
    ...
    if (!batched) {
        const creationHash = await ctx.transactions.writeContract({
            address: ctx.selfProfile.userContract,
            abi: userContractAbi,
            functionName: "addMessage",
            args: [creationBox, creationTag],
        });
...
export async function sendChatMessage(ctx, input) {
    ...
    const encrypted = await aesEncrypt(input.chat.chatKey, encodePayload(createMessagePayload(text, attachments)));
...
export async function inviteChatMember(ctx, input) {
    const target = input.target.trim();
    if (!target) {
        throw new Error("Invite target is empty");
    }
    const invitedUser = target.startsWith("0x") && target.length === 42
        ? await readUserByAddress(ctx, toAddress(target))
        : await readUserByLogin(ctx, target);
    if (!invitedUser) {
        throw new Error("User not found");
    }
    const invitationEvent = await aesEncrypt(input.chat.chatKey, encodePayload(createInvitationPayload({
        invited: toAddress(invitedUser.userAddress),
    })));
    const invitationTag = await generateMessageTag(input.chat.chatKey);
    const mainInvitation = await rsaEncrypt(invitedUser.pubkey, encodePayload(createMainInvitationPayload({
